#ifndef LZARCommpress_H
#define LZARCommpress_H
...
namespace LZAR
{
    enum{ N=4096,  // size of ring buffer
          ...
          NIL=4096,	// index for root of binary search trees
          M=15 };
    ...
    private:
        inline void PutBit(int bit)  /* Output one bit (bit = 0,1) */
        {
            if (bit) buffer |= mask;
            if ((mask >>= 1) == 0) {
                Arc->putbyte(buffer);
        ...
        inline void FlushBitBuffer(void)  /* Send remaining bits */
        {
            for (int i = 0; i < 7; i++) PutBit(0);
        }

        inline int GetBit(void)  /* Get one bit (0 or 1) */
        {
            if ((mask >>= 1) == 0) {
                buffer = Arc->getbyte();  mask = 128;
            }
    ...
    public:
        void Encode(MyIOBase *Source)
        {
            ...
            for (len = 0; len < F && (c = Source->getbyte()) != EOF; len++)
                text_buf[r + len] = c;
//            textsize = len;
        ...
        void Decode(MyIOBase *Source,unsigned long textsize)
        {
            int  i, j, k, r, c;
...
class LZARCoder : public CoderBase //Encode class
{
private:
    LZAR::Coder C;
    ...
    {
        C.init(Archive,Mode);
        C.Decode(Source,Head.usize);
    }
public:
    LZARCoder(bool m,MyIOBase *p=NULL):CoderBase(p)
    {
      MathodN="LZAR";
      Mode=m;
    }
